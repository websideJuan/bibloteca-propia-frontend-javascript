...
let user;
(async () => {
  const cookieUser = await getCookieStore();
  user = JSON.parse(cookieUser?.value || null);
})();
...
aboutPage.init = function handelAboutPage() {
  ...
  document.querySelector(".container").addEventListener("click", (event) => {
    if (event.target.classList.contains("log-out")) {
      location.hash = "#/logout"; // Redirect to login page after logout
    }
  });
};
