    ...
    <main style="height: calc(100dvh - 50px);">
      <div class="glass-effect" style="height: 100%; scrollbar-width: none; overflow-y: auto; padding: 15px 0;">
        ...
        <div class="subjects-container" style="display: grid; grid-template-columns: repeat(4, minmax(100vw, 1fr));  overflow-y: auto;scrollbar-width: none; scroll-snap-type: x mandatory; scroll-behavior: smooth;">
          ...
          ${Object.keys(m.materias).map((a,e)=>`
          <div class="subject" style="transform:${e!==0?"scale(.8)":"scale(1)"}; scroll-snap-align: center; transition: transform 0.3s ease; display: flex; justify-content: center; align-items: center;"> 
            <div class="card glass-effect">
              <img src="${m.materias[a].imagen}" width="100" height="70" alt="${a}" class="subject-image">
              <h3 class="subject-title">${m.materias[a].nombre}</h3>
            </div>
          </div>
          `).join("")}
        </div>
            
  `}P.init=function(){const e=document.querySelector(".subjects-container"),r=new IntersectionObserver(t=>{t.forEach(s=>{s.isIntersecting?s.target.style.transform="scale(1)":s.target.style.transform="scale(.8)"})},{threshold:.5});e.querySelectorAll(".subject").forEach(t=>{r.observe(t)}),console.log("Home Page Initialized")};class d{static handleFormSubmit(e,r){e.addEventListener("submit",t=>{t.preventDefault();const s=new FormData(t.target),o=Object.fromEntries(s.entries());if(e.querySelectorAll(".error-message").forEach(n=>{!n.textContent===""||o[n.id.replace("-error","")].length!==0||(console.log(n.textContent===""),n.textContent=`this ${n.id.replace("-error","")} is required`)}),!!this.stateOfValidation.isValid){this.stateBtn=!0;try{this.stateBtn&&(e.querySelector("button[type='submit']").disabled=!0,e.querySelector("button[type='submit']").innerHTML='<p class="spinner"></p>'),r(o)}catch(n){throw new Error(`Error processing form: ${n}`)}finally{this.stateBtn=!1}}})}static validateInput(e,r){const t=document.getElementById(`${e.name}-error`);e.addEventListener("input",()=>{const s=e.value;try{r(s),t.textContent="",this.stateOfValidation.isValid=!0}catch(o){t.textContent=o.message,this.stateOfValidation.isValid=!1}})}static clearForm(e){const r=document.getElementById(e);r?(r.reset(),r.querySelectorAll(".error-message").forEach(s=>s.textContent="")):console.error(`Form with ID ${e} not found.`)}}p(d,"stateOfValidation",{isValid:!1,stateBtn:!1});class T{static popup(e,r="info",t=6e3){const s=document.createElement("div");s.className=`notification notification-${r}`,s.textContent=e,document.body.appendChild(s),setTimeout(()=>{s.remove()},t)}}class B{constructor(){this.users=[]}addUser(e){if(!e||!e.username||e.password,!e.email)throw new Error("Invalid user data");try{const r=this.getAllUsers();r&&(this.users=r),this.users.push(e),localStorage.setItem("users",JSON.stringify(this.users))}catch(r){this.errors=[],this.errors.push(r.message),console.error("Error adding user:",r)}}getUser(e){const r=this.getAllUsers();return r?r.find(t=>t.username===e):null}getAllUsers(){return JSON.parse(localStorage.getItem("users"))}clearUsers(){this.users=[]}}const E=new B;function U(){return`
    <div class="container">
      <h1>
        Iniciar sesión
